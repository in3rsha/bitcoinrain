...
function decodeblockheader($blockheader) {

	$header['hash'] = swapEndian(hash('sha256', hash('sha256', hex2bin($blockheader), true)));
	
	$header['version']    =	hexdec(swapEndian(substr($blockheader, 0, 8)));
	$header['prevblock']  =	swapEndian(substr($blockheader, 8, 64)); // searchable byte order
	$header['merkleroot'] =	swapEndian(substr($blockheader, 72, 64));
	$header['timestamp']  =	hexdec(swapEndian(substr($blockheader, 136, 8)));
	$header['bits']       =	swapEndian(substr($blockheader, 144, 8));
	$header['nonce']      =	hexdec(swapEndian(substr($blockheader, 152, 8)));
...
function decodeblock($payload) {
	$header = substr($payload, 0, 160);
	$txcount = varInt(substr($payload, 160));
	$size = strlen($payload)/2;
	
	$result = decodeblockheader($header);
	$result['txcount'] = $txcount[1];
	$result['size'] = $size;
	$result['type'] = 'block';
