...
function connect($version, $node_ip, $node_port, $local_ip, $local_port, $testnet) {
	...
	echo "Connecting to $node_ip:$node_port...\n";
...
while (true) {
	...
	while ($connected == false) {
		...
		if ($socket) { // if we have a socket resource (and not "false" from a failed connection)
			$connected = true; // allow us to break out of this lool
		}
		else {
	...
	// If we have sent 2 or more pings with no response...
	if (count($pings) > 2) {
		echo "\nNo response to 2 pings. Attempting reconnect in 5s...\n";
